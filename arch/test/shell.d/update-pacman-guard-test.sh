...
run_guard() {
  STRAPD_PACMAN_CMDLINE="$1" "$ROOT/bin/strapd-update-pacman-guard"
}
...
grep -q 'strapd update' "$test_tmp/direct.err" || fail "pacman guard explains strapd update entrypoint"
...
STRAPD_UPDATE_PACMAN=1 run_guard "pacman -Syu --noconfirm" >"$test_tmp/strapd.out" 2>"$test_tmp/strapd.err"
[[ ! -s $test_tmp/strapd.err ]] || fail "pacman guard stays quiet for strapd update pacman call"
pass "pacman guard allows strapd update pacman call"

STRAPD_ALLOW_DIRECT_PACMAN=1 run_guard "pacman -Syu --noconfirm" >"$test_tmp/override.out" 2>"$test_tmp/override.err"
