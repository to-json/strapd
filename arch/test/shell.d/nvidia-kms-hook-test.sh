...
hooks_conf="$ROOT/etc/mkinitcpio.conf.d/strapd_hooks.conf"
...
# applied, and prints the resulting HOOKS. mkinitcpio does not run under set -u,
# but the config must survive it, so source under it anyway.
resolved_hooks() {
  local modules_decl=""
  ...
  STRAPD_PCI_DEVICES_PATH="$tmp_dir/devices" bash -uc "
    FILES=()
    XKBLAYOUT=us
    $modules_decl
    source '$hooks_conf'
...
# Even next to a readable NVIDIA GPU, since the unreadable device may be the iGPU.
write_pci_devices 0x10de:0x030000
