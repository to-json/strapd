...
snapshot="$ROOT/bin/strapd-snapshot"
...
exit 1
STUB
chmod +x "$fake_bin/strapd-cmd-missing"

cat >"$fake_bin/strapd-version" <<'STUB'
#!/bin/bash
echo 4.0.0
STUB
chmod +x "$fake_bin/strapd-version"
...
grep -qF 'strapd-snapshot create || (($? == 127))' "$ROOT/bin/strapd-update" ||
