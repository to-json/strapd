...
# records its arguments and which streams reached a terminal: pacman puts its
# questions on stderr once it is not running --noconfirm.
...
# ":: ... Remove qemu-block-gluster? [y/N]" is deliberately absent: under
# --noconfirm it goes to stdout, so nothing downstream can be built on it.
...
update_env() {
  printf '%s\n' \
    "STRAPD_REPLACED_DIR=$test_tmp/replaced" \
    "PACMAN_ATTEMPTS=$test_tmp/attempts" \
    "PACMAN_CALLS=$test_tmp/calls" \
    "CONFLICT_REPORT=$test_tmp/report" \
    "OWNED_PATHS=" \
    "STRAPD_UPDATE_UNATTENDED=${STRAPD_UPDATE_UNATTENDED:-}" \
    "STRAPD_UPDATE_INTERACTIVE=${STRAPD_UPDATE_INTERACTIVE:-}" \
...
run_headless() {
  ...
  env "${environment[@]}" bash "$ROOT/bin/strapd-update-system-pkgs" \
    </dev/null >"$test_tmp/out" 2>"$test_tmp/err"
}

# script gives the update the pty strapd-update always runs it on, so the
# terminal checks see what a person at the keyboard would give them. $1
# optionally takes one stream back off the pty.
run_on_terminal() {
  mapfile -t environment < <(update_env)
  env "${environment[@]}" \
    script -qec "bash '$ROOT/bin/strapd-update-system-pkgs' ${1:-}" "$test_tmp/out" >/dev/null 2>&1
}
...
# carries the question once --noconfirm is gone, stdin carries the answer.
write_conflict_report
...
grep -q 'strapd update' "$test_tmp/err" ||
...
write_conflict_report
if STRAPD_UPDATE_UNATTENDED=1 run_on_terminal; then
...
write_conflict_report
if STRAPD_UPDATE_INTERACTIVE=1 run_headless; then
