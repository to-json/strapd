...
run_update() {
  STRAPD_REPLACED_DIR="$replaced" \
    ...
    bash "$ROOT/bin/strapd-update-system-pkgs"
}
...
fresh_work
stray="$work/strapd-fcitx5.service"
echo "stray content" >"$stray"
write_report strapd-settings-dev "$stray"
...
# A real fight between packages, not strapd's leftovers. pacman appends
# "(owned by ...)" here.
fresh_work
echo "theirs" >"$stray"
write_report strapd-settings-dev "$stray" someone-else
...
fresh_work
echo "stray" >"$stray"
write_report strapd-chromium-bin "$stray"
if run_update >"$test_tmp/out" 2>"$test_tmp/err"; then
  fail "an optional strapd-prefixed package gets its conflicts auto-resolved"
fi
pass "only the packages that own system paths get their conflicts resolved"

# Not strapd's conflict to resolve.
fresh_work
echo "stray" >"$stray"
...
pass "a conflict from a non-strapd package is left for a human"

# The path is used literally, so glob characters in a name mean nothing.
fresh_work
globby="$work/strapd-[1].conf"
echo "globby" >"$globby"
write_report strapd-settings-dev "$globby"
run_update >"$test_tmp/out" 2>"$test_tmp/err" ||
...
fresh_work
conflict_dir="$work/strapd-dir"
mkdir -p "$conflict_dir"
write_report strapd-settings-dev "$conflict_dir"
run_update >"$test_tmp/out" 2>"$test_tmp/err" ||
...
fresh_work
spaced="$work/strapd theme.conf"
echo "spaced" >"$spaced"
write_report strapd-settings-dev "$spaced"
run_update >"$test_tmp/out" 2>"$test_tmp/err" ||
...
echo "ours" >"$stray"
write_raw_report \
  "strapd-settings-dev: $stray exists in filesystem" \
...
echo "ours" >"$stray"
write_report strapd-settings-dev "$stray"
if PATH="$stub_bin:$ROOT/bin:$PATH" STRAPD_REPLACED_DIR="$replaced" \
  bash "$ROOT/bin/strapd-update-system-pkgs-when-conflicted" "$test_tmp/report" \
