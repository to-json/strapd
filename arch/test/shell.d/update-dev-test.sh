...
run_dev_update() {
  STRAPD_PATH="$1" \
    ...
    PATH="$stub_bin:$PATH" \
    "$ROOT/bin/strapd-update-dev"
}

: >"$git_log"
run_dev_update /usr/share/strapd
...
grep -F "STRAPD_PATH is not a git checkout: $checkout" "$test_tmp/invalid.err" >/dev/null ||
...
grep -qE '^ *strapd-update-dev$' "$ROOT/bin/strapd-update" ||
