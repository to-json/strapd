...
test_bin=$(mktemp -d)
log_file=$(mktemp)
hook_path="$test_home/.config/strapd/hooks/post-update.d/setup-agent.hook"
...
cat >"$test_bin/strapd-notification-send" <<'EOF'
#!/bin/bash
...
EOF
chmod +x "$test_bin/strapd-notification-send"
...
[[ -f $test_home/.local/state/strapd/done/agent-setup-invitation ]] || fail "agent invitation records completion"
...
grep -qx 'exec:strapd menu summon setup.default.agent' "$log_file" ||
...
fresh_home=$(mktemp -d)
mkdir -p "$fresh_home/.config/strapd/defaults"
printf 'claude\n' >"$fresh_home/.config/strapd/defaults/agent"
: >"$log_file"
cp "$ROOT/install/user/first-run/setup-agent.hook" "$hook_path"
...
[[ ! -f $fresh_home/.local/state/strapd/done/agent-setup-invitation ]] ||
