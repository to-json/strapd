#!/bin/bash
#
# The install scripts that grant group memberships must record them in the
# provisioning groups file, for first-boot user creation and factory reset, and
# only call usermod when the install user actually exists.
#
# Docker is deliberately excluded: the group is root-equivalent, so it is opt-in
# through strapd-setup-security-sudoless-docker.
...
export STRAPD_PROVISIONING_DIR="$TMPDIR/provisioning"
...
STRAPD_INSTALL_USER="" bash -eE "$ROOT/install/config/docker.sh"
STRAPD_INSTALL_USER="" bash -eE "$ROOT/install/hardware/input-group.sh"

[[ -f $STRAPD_PROVISIONING_DIR/groups ]] || fail "groups file written without an install user"
grep -qxF input "$STRAPD_PROVISIONING_DIR/groups" || fail "input group recorded"
...
! grep -qxF docker "$STRAPD_PROVISIONING_DIR/groups" || fail "docker group must not be recorded"
pass "docker group is not recorded at install"

# Missing user (defensive): no usermod either.
STRAPD_INSTALL_USER=ghost bash -eE "$ROOT/install/hardware/input-group.sh"
...
STRAPD_INSTALL_USER="" bash -eE "$ROOT/install/hardware/input-group.sh"
[[ $(grep -cxF input "$STRAPD_PROVISIONING_DIR/groups") == 1 ]] || fail "input group recorded once"
pass "group recording is idempotent"

# Existing user: usermod applies the recorded groups, and docker is never among them.
STRAPD_INSTALL_USER=existing bash -eE "$ROOT/install/config/docker.sh"
STRAPD_INSTALL_USER=existing bash -eE "$ROOT/install/hardware/input-group.sh"
