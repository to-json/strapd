...
cat >"$mock_bin/strapd-done" <<'SH'
#!/bin/bash
[[ $1 == "check" && $2 == "first-run-user" ]]
SH
cat >"$mock_bin/strapd-provision-user" <<'SH'
#!/bin/bash
touch "$STRAPD_TEST_FINALIZE_CALLED"
SH
chmod +x "$mock_bin/strapd-done" "$mock_bin/strapd-provision-user"

finalize_called="$test_tmp/finalize-called"
HOME="$test_tmp/home" PATH="$mock_bin:$PATH" STRAPD_TEST_FINALIZE_CALLED="$finalize_called" \
  bash "$ROOT/bin/strapd-provision-first-run" >"$test_tmp/output"
...
if grep -F 'user-migration-notify-watch-enabled' "$ROOT/bin/strapd-provision-first-run" >/dev/null; then
