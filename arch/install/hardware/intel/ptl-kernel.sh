...
if strapd-hw-match "XPS" && strapd-hw-intel-ptl; then
  echo "Detected Dell XPS Panther Lake, installing PTL kernel..."

  strapd-pkg-add linux-ptl linux-ptl-headers
  ...
  # Named to sort after strapd-defaults.conf: drop-ins are read in order and
