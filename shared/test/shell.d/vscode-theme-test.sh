...
CURRENT_THEME="$TEST_HOME/.local/state/strapd/current/theme"
mkdir -p "$FAKE_BIN" "$CURRENT_THEME"

cat >"$FAKE_BIN/strapd-cmd-present" <<'EOF'
#!/bin/bash
printf '%s\n' "$1" >>"$EDITOR_PROBE_LOG"
exit 1
EOF

cat >"$FAKE_BIN/strapd-toggle-enabled" <<'EOF'
#!/bin/bash
exit 1
EOF
...
EDITOR_PROBE_LOG="$TEST_HOME/editor-probes.log" \
  ...
  HOME="$TEST_HOME" \
  "$ROOT/bin/strapd-theme-set-vscode"
