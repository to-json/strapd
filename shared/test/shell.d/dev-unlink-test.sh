...
printf 'sudo' >>"$STRAPD_DEV_UNLINK_TEST_LOG"
for arg in "$@"; do
  printf '\t%s' "$arg" >>"$STRAPD_DEV_UNLINK_TEST_LOG"
done
printf '\n' >>"$STRAPD_DEV_UNLINK_TEST_LOG"

if [[ $1 == "tee" ]]; then
  cat >"$STRAPD_DEV_UNLINK_TEST_CONF"
fi
SH
...
printf 'gum' >>"$STRAPD_DEV_UNLINK_TEST_LOG"
for arg in "$@"; do
  printf '\t%s' "$arg" >>"$STRAPD_DEV_UNLINK_TEST_LOG"
done
printf '\n' >>"$STRAPD_DEV_UNLINK_TEST_LOG"
...
run_unlink() {
  HOME="$test_tmp/home" \
    STRAPD_DEV_UNLINK_TEST_LOG="$log_file" \
    STRAPD_DEV_UNLINK_TEST_CONF="$conf_file" \
    PATH="$stub_bin:$PATH" \
    "$ROOT/bin/strapd-dev-unlink" "$@"
}

: >"$log_file"
run_unlink --no-reboot

grep -Fx $'sudo\ttee\t/etc/strapd.conf' "$log_file" >/dev/null ||
  fail "dev unlink writes the package path without rebooting" "$(cat "$log_file")"
[[ $(<"$conf_file") == 'export STRAPD_PATH="/usr/share/strapd"' ]] ||
  fail "dev unlink writes the package path guard" "$(<"$conf_file")"
pass "dev unlink writes the package path guard"

# Left behind, it keeps sudo running a checkout nothing else points at.
grep -Fx $'sudo\trm\t-f\t/etc/sudoers.d/strapd-dev-path' "$log_file" >/dev/null ||
...
grep -F 'Usage: strapd dev unlink [--no-reboot]' "$test_tmp/invalid.err" >/dev/null ||
