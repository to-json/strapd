...
cat >"$WORKDIR/bin/strapd-notification-send" <<SH
#!/bin/bash
...
receive() {
  ...
  : >"$WORKDIR/notifications"
  PATH="$WORKDIR/bin:$PATH" "$@" "$ROOT/bin/strapd-tailscale-receive" --once "$downloads"
...
# The shell keeps the click command with the toast, so receiving does not sit
# blocked on an answer and the toast still opens the file after a restart. The
# path rides as its own discrete --exec argument, so it is literal data.
...
[[ -z $(ls -A "$downloads/.strapd-taildrop") ]] ||
  fail "taildrop receive empties its staging directory" "$(ls -A "$downloads/.strapd-taildrop")"
