...
animation="$ROOT/bin/strapd-branding-about-animation"
source "$animation"
...
# Every frame is the same logo in different colours. One that changed a
# character, reached past the logo, or lit more than the band is wide would be
# drawing over whatever the caller put beside it.
esc=$'\e'
...
# Terminals that render bold text in bright colours -- foot's
# bold-text-in-bright -- map a bold regular colour to its bright counterpart, so
# a band using one of those vanishes into a logo drawn in the matching regular
# colour.
...
# Everything that breaks that is refused by the one check.
write_logo '$1████' '  ████'
