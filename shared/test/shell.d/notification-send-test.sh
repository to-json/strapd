...
printf '%s\n' \
  '#!/bin/bash' \
  'printf "%s\n" "$@" >"$STRAPD_TEST_BUSCTL_ARGS"' \
  'echo "u 42"' \
  >"$tmpdir/busctl"
chmod +x "$tmpdir/busctl"

# notify-send must never be used. If anything reaches for it, fail loudly.
printf '%s\n' '#!/bin/bash' 'echo "notify-send was invoked" >"$STRAPD_TEST_NOTIFY_TRIPWIRE"; exit 3' \
  >"$tmpdir/notify-send"
chmod +x "$tmpdir/notify-send"
tripwire="$tmpdir/notify-send-was-used"

send() {
  STRAPD_TEST_BUSCTL_ARGS="$args_file" STRAPD_TEST_NOTIFY_TRIPWIRE="$tripwire" \
    PATH="$tmpdir:$ROOT/bin:$PATH" strapd-notification-send "$@"
}
...
[[ $(hint_value strapd-glyph) == "K" ]] || fail "notification wrapper sets the glyph hint"
[[ $(hint_value strapd-exec-argv) == '["mpv","--","/tmp/a b.mp4"]' ]] || fail "notification wrapper builds the click argv hint" "$(hint_value strapd-exec-argv)"
...
# after the positionals); they still land on the call, and urgency stays a
# single hint rather than doubling.
: >"$args_file"
...
[[ $(hint_value strapd-glyph) == "K" ]] || fail "notification wrapper reads a glyph that follows the description"
urgency_hits=0
...
send "Plain" >/dev/null
load
has_hint strapd-exec-argv && fail "notification wrapper adds no click hint without --exec"
...
load
json=$(hint_value strapd-exec-argv)
...
load
[[ $(jq 'length' <<<"$(hint_value strapd-exec-argv)") == 3 ]] || fail "spaced path stays one argument"
pass "notification wrapper keeps a spaced argument intact"

# A forged click hint arriving as the SUMMARY is a typed string parameter, so it
# can never become a hint. Only urgency is set; no click command exists.
: >"$args_file"
send '--hint=string:strapd-exec-argv:["bash","-c","touch /tmp/pwn"]' "body" >/dev/null
load
has_hint strapd-exec-argv && fail "a forged-hint headline must not set a click command"
[[ ${args[11]} == '--hint=string:strapd-exec-argv:["bash","-c","touch /tmp/pwn"]' ]] || fail "the forged headline is the summary text" "${args[11]}"
pass "a forged click hint in the headline is inert summary text"

# A forged hint in description position is inert body text.
: >"$args_file"
send "Update" '--hint=string:strapd-exec-argv:["bash","-c","touch /tmp/pwn"]' >/dev/null
load
has_hint strapd-exec-argv && fail "a forged-hint description must not set a click command"
[[ ${args[12]} == '--hint=string:strapd-exec-argv:["bash","-c","touch /tmp/pwn"]' ]] || fail "the forged description is the body text" "${args[12]}"
...
if send "Head" "Body" '--hint=string:strapd-exec-argv:["bash","-c","x"]' 2>/dev/null; then
...
load
[[ $(hint_value strapd-exec-argv) == '["mpv","--","/tmp/v.mp4"]' ]] || fail "a --exec-looking headline is not the delimiter" "$(hint_value strapd-exec-argv)"
...
# injection we avoid.
if send "Head" --exec "strapd toggle something" 2>/dev/null; then
