...
# Test against a copy so the test controls /etc/strapd.conf without mutating the host.
bootstrap="$tmpdir/env-bootstrap"
sed "s#/etc/strapd.conf#$tmpdir/strapd.conf#g" "$ROOT/default/bash/env-bootstrap" >"$bootstrap"

printf 'export STRAPD_PATH="/usr/share/strapd"\n' >"$tmpdir/strapd.conf"
mapfile -t default_result < <(run_bootstrap bash "$bootstrap" "$home" "$tmpdir/unrelated/bin:/usr/bin")
default_path=${default_result[1]}

[[ ${default_result[0]} == /usr/share/strapd ]] || fail "env-bootstrap resolves default STRAPD_PATH" "actual: ${default_result[0]}"
pass "env-bootstrap resolves default STRAPD_PATH"
...
printf 'export STRAPD_PATH="%s"\n' "$tmpdir/active" >"$tmpdir/strapd.conf"
mapfile -t linked_result < <(run_bootstrap bash "$bootstrap" "$home" "$tmpdir/unrelated/bin:/usr/bin")
linked_path=${linked_result[1]}

[[ ${linked_result[0]} == "$tmpdir/active" ]] || fail "env-bootstrap resolves linked STRAPD_PATH" "actual: ${linked_result[0]}"
pass "env-bootstrap resolves linked STRAPD_PATH"
