...
hybrid_gpu() {
  PATH="$fake_bin:$PATH" timeout --kill-after=1s 10s bash "$ROOT/bin/strapd-hw-hybrid-gpu"
}
...
cat >"$fake_bin/strapd-cmd-present" <<'STUB'
#!/bin/bash
exit 1
STUB
chmod +x "$fake_bin/strapd-cmd-present"
