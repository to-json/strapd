#!/bin/bash
#
# strapd-sudo-docker is the single answer to "does Docker need sudo", and it
# answers two questions on purpose. The default asks whether this session can
# reach the socket, which decides if a command must elevate; --configured asks
# whether the account is set up for it, which is what the menu needs. Between
...
command="$ROOT/bin/strapd-sudo-docker"
...
# A writable stand-in means the socket is reachable, an unwritable one that it
# is not: the file mode rather than a live daemon.
...
run() { # SOCKET GROUPS [--configured]
  env PATH="$TMPDIR/bin:$PATH" STRAPD_DOCKER_SOCKET="$1" STUB_GROUPS="$2" USER=tester \
    bash "$command" ${3:+"$3"}
}
...
# The window this split exists for: the account carries the group while the
# running session still cannot use it. The menu must offer Remove while
# lazydocker and the Windows VM must still prompt.
