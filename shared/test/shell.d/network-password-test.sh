...
run_failure_case() {
  ...
  if PATH="$tmp/bin:$PATH" "$ROOT/bin/strapd-network-password" wlan0 >"$tmp/output" 2>"$tmp/error"; then
...
output=$(PATH="$tmp/bin:$PATH" "$ROOT/bin/strapd-network-password" wlan0)
