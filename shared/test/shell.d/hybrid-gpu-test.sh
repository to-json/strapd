...
cat >"$fake_bin/strapd-cmd-missing" <<'STUB'
#!/bin/bash
exit 1
STUB
...
TEST_TMP="$test_tmp" SUCCEED_ON_ATTEMPT=3 \
  PATH="$fake_bin:$PATH" bash "$ROOT/bin/strapd-toggle-hybrid-gpu" >/dev/null
...
error=$(
  TEST_TMP="$test_tmp" \
    PATH="$fake_bin:$PATH" bash "$ROOT/bin/strapd-toggle-hybrid-gpu" 2>&1 >/dev/null
)
status=$?
set -e
...
set +e
output=$(TEST_TMP="$test_tmp" PATH="$fake_bin:$PATH" timeout 25s bash "$ROOT/bin/strapd-toggle-hybrid-gpu" 2>&1)
status=$?
set -e
