...
TEST_HOME=$(mktemp -d)
FAKE_STRAPD=$(mktemp -d)
trap 'rm -rf "$TEST_HOME" "$FAKE_STRAPD"' EXIT

mkdir -p "$FAKE_STRAPD/bin"

cat >"$FAKE_STRAPD/bin/strapd-agent-usage-good" <<'EOF'
#!/bin/bash
echo '{"schemaVersion":1,"id":"good","name":"Good Agent","totalPrompts":3}'
EOF

cat >"$FAKE_STRAPD/bin/strapd-agent-usage-noisy" <<'EOF'
#!/bin/bash
echo "this is not json"
EOF

cat >"$FAKE_STRAPD/bin/strapd-agent-usage-skipped" <<'EOF'
#!/bin/bash
echo '{"id":"skipped"}'
EOF

# The updater itself lives in the same namespace as the collectors it globs.
cat >"$FAKE_STRAPD/bin/strapd-agent-usage-update" <<'EOF'
#!/bin/bash
echo '{"id":"update"}'
EOF

chmod +x "$FAKE_STRAPD/bin/"strapd-agent-usage-*

usage_dir="$TEST_HOME/.local/state/strapd/agents/usage"

HOME="$TEST_HOME" STRAPD_PATH="$FAKE_STRAPD" XDG_STATE_HOME="" \
  "$ROOT/bin/strapd-agent-usage-update" --except skipped 2>/dev/null && fail "update reports a failing collector"
...
HOME="$TEST_HOME" STRAPD_PATH="$FAKE_STRAPD" XDG_STATE_HOME="" \
  "$ROOT/bin/strapd-agent-usage-update" skipped 2>/dev/null ||
